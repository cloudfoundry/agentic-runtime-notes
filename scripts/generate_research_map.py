...
def generate_html(
    notes: list[Note], plots: dict, primitives: list[dict], use_cases: list[dict] | None = None
) -> str:
    ...
    for use_case in use_cases:
        for relationship in ("core", "supporting"):
            for path in use_case[relationship]:
                use_case_memberships.setdefault(path, {})[use_case["id"]] = relationship
    use_case_membership_data = json.dumps(use_case_memberships, ensure_ascii=True).replace("</", "<\\/")
    note_titles = {note.path.as_posix(): note.title for note in notes}
    primitive_titles = {primitive["id"]: primitive["title"] for primitive in primitives}
    primitive_title_data = json.dumps(primitive_titles, ensure_ascii=True).replace("</", "<\\/")
    use_case_title_data = json.dumps(
        {use_case["id"]: use_case["title"] for use_case in use_cases}, ensure_ascii=True
    ).replace("</", "<\\/")
    use_case_cards = []
    for use_case in use_cases:
        use_case_id = html.escape(use_case["id"])
        evidence = []
        for relationship in ("core", "supporting"):
            links = "".join(
            ...
            evidence.append(f'<section><h4>{relationship.title()}</h4><ul>{links}</ul></section>')
        capabilities = "".join(f'<li>{html.escape(value)}</li>' for value in use_case["unique_capabilities"])
        decisions = "".join(f'<li>{html.escape(value)}</li>' for value in use_case["rfc_decisions"])
        applicability = "".join(
            f'<li data-primitive-applicability="{html.escape(primitive_id)}"><strong>{html.escape(primitive_titles.get(primitive_id, primitive_id))}:</strong> <span>{html.escape(value.title())}</span></li>'
            for primitive_id, value in use_case["primitive_applicability"].items()
        )
        use_case_cards.append(f'''<article class="use-case-card">
<button id="use-case-{use_case_id}" class="use-case-select" type="button" data-use-case="{use_case_id}" aria-pressed="false"><span class="use-case-title">{html.escape(use_case["title"])}</span><span>{html.escape(use_case["workshop_outcome"])}</span><span class="candidate-poc"><strong>Candidate POC:</strong> {html.escape(use_case["poc"])}</span></button>
<details class="use-case-details"><summary>Actors, lifecycle, decisions, and evidence</summary><dl><dt>Primary actor</dt><dd>{html.escape(use_case["primary_actor"])}</dd><dt>Beneficiary</dt><dd>{html.escape(use_case["beneficiary"])}</dd><dt>Lifecycle</dt><dd>{html.escape(use_case["lifecycle"])}</dd><dt>Authority boundary</dt><dd>{html.escape(use_case["authority_boundary"])}</dd><dt>Failure domain</dt><dd>{html.escape(use_case["failure_domain"])}</dd></dl><section><h4>Unique capabilities</h4><ul>{capabilities}</ul></section><section><h4>RFC decisions</h4><ul>{decisions}</ul></section><section><h4>Primitive applicability</h4><ul>{applicability}</ul></section><div class="use-case-links">{''.join(evidence)}</div></details></article>''')
...
<title>Agentic Runtime Workshop Results</title><style>
:root {{ --selected-accent:#c69cff; color-scheme: dark; font-family: Inter, ui-sans-serif, system-ui, sans-serif; background:#101619; color:#e7f1ed; }}
...
.section-heading {{ margin-top:44px; }} .card-actions {{ display:flex; justify-content:flex-end; margin-bottom:12px; }} .use-case-grid {{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:18px; }} .use-case-card {{ min-width:0; background:#15211e; border:1px solid #3d5750; border-top:4px solid #c69cff; border-radius:14px; overflow:hidden; }} .use-case-select {{ display:flex; flex-direction:column; gap:12px; width:100%; min-height:100%; padding:22px; color:inherit; background:#1b2725; border:0; text-align:left; cursor:pointer; }} .use-case-select:hover,.use-case-select:focus-visible,.use-case-select[aria-pressed="true"] {{ background:#283633; box-shadow:inset 0 0 0 1px #c69cff; }} .use-case-title {{ color:#d9bdff; font-size:1.3rem; font-weight:800; line-height:1.25; }} .candidate-poc {{ margin-top:auto; color:#d8d0df; }} .use-case-details {{ border-top:1px solid #304640; padding:14px 20px 18px; }} .use-case-details summary {{ color:#d9bdff; cursor:pointer; font-weight:700; }} .use-case-details dl {{ margin-bottom:0; }} .use-case-details dt {{ margin-top:14px; color:#b4c8c0; font-weight:700; }} .use-case-details dd {{ margin:4px 0 0; color:#d5e3de; line-height:1.45; }} .use-case-links {{ display:grid; grid-template-columns:1fr 1fr; gap:16px; }} .use-case-links ul {{ margin:8px 0 0; padding-left:18px; }} .use-case-links li {{ margin:6px 0; }}
...
@media(max-width:800px) {{ .use-case-grid,.primitive-grid {{ grid-template-columns:1fr; }} .use-case-select,.primitive-select {{ min-height:0; }} }}
@media(max-width:600px) {{ body {{ padding:24px 14px; }} .use-case-links,.primitive-links {{ grid-template-columns:1fr; gap:0; }} .matrix-tabs {{ margin-left:-14px; margin-right:-14px; padding-left:14px; padding-right:14px; }} .map {{ height:720px; margin-left:58px; }} .axis-y {{ left:-40px; }} }}
</style></head><body><main><p class="intro">Agentic Runtime Working Group</p><h1>Workshop results</h1>
<p class="intro">This page summarizes the workshop results: two focus use cases, three candidate platform primitives, and the collected evidence behind them.</p>
<h2 class="section-heading">Focus use cases</h2><div class="card-actions"><button class="show-all use-case-show-all" type="button">Show all use cases</button></div><div class="use-case-grid">{''.join(use_case_cards)}</div><h2 class="section-heading">Candidate platform primitives</h2><div class="card-actions"><button class="show-all primitive-show-all" type="button">Show all primitives</button></div><div class="primitive-grid">{''.join(cards)}</div><p id="filter-status" role="status" aria-live="polite"></p><div class="legend"><span>Research</span><span class="idea-key">Idea</span></div><div class="matrix-tabs" role="tablist" aria-label="Rating matrices">{''.join(tabs)}</div>{''.join(matrices)}</main>
<dialog id="detail" aria-labelledby="dialog-title"><button class="close" aria-label="Close">X</button><div id="content"></div></dialog>
<script>const plots={data};const primitiveMemberships={membership_data};const useCaseMemberships={use_case_membership_data};const primitiveTitles={primitive_title_data};const useCaseTitles={use_case_title_data};const dialog=document.querySelector('#detail'),content=document.querySelector('#content'),filterStatus=document.querySelector('#filter-status');let selectedUseCase=null,selectedPrimitive=null;document.querySelector('.close').onclick=()=>dialog.close();
 function escapeHtml(value){{return String(value).replace(/[&<>"']/g,character=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[character]))}}
 function noteMatchesSelection(noteId){{const matchesUseCase=!selectedUseCase||useCaseMemberships[noteId]?.[selectedUseCase];const matchesPrimitive=!selectedPrimitive||primitiveMemberships[noteId]?.[selectedPrimitive];return Boolean(matchesUseCase&&matchesPrimitive)}}
 function updateMarkers(){{const hasSelection=Boolean(selectedUseCase||selectedPrimitive);document.querySelectorAll('.marker').forEach(m=>{{if(m.dataset.noteIds){{const noteIds=JSON.parse(m.dataset.noteIds);const matchingCount=hasSelection?noteIds.filter(noteMatchesSelection).length:0;m.textContent=hasSelection?`${{matchingCount}}/${{noteIds.length}}`:String(noteIds.length);m.setAttribute('aria-label',hasSelection?`${{matchingCount}} of ${{noteIds.length}} matching notes at this position`:`${{noteIds.length}} notes at this position`);m.classList.toggle('related',matchingCount>0);m.classList.toggle('dimmed',Boolean(hasSelection&&!matchingCount))}}else{{const related=noteMatchesSelection(m.dataset.id);m.classList.toggle('related',hasSelection&&related);m.classList.toggle('dimmed',hasSelection&&!related)}}}});const hasMatches=Object.values(plots).some(notes=>notes.some(note=>noteMatchesSelection(note.id)));filterStatus.textContent=selectedUseCase&&selectedPrimitive&&!hasMatches?'No directly linked evidence for this use-case and primitive combination.':''}}
 function updateUseCaseControls(){{document.querySelectorAll('.use-case-select').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.useCase===selectedUseCase)))}}
 ...
 function selectUseCase(button){{selectedUseCase=selectedUseCase===button.dataset.useCase?null:button.dataset.useCase;updateUseCaseControls();updateMarkers()}}
 function clearUseCaseSelection(){{selectedUseCase=null;updateUseCaseControls();updateMarkers()}}
...
function relationshipBadge(relationship,label){{return relationship?`<span class="primitive-badge">${{escapeHtml(label)}} ${{relationship==='core'?'Core':'Supporting'}}</span>`:''}}
 function relationshipBadges(note){{const useCaseRelationship=selectedUseCase&&useCaseMemberships[note.id]?.[selectedUseCase];const primitiveRelationship=selectedPrimitive&&primitiveMemberships[note.id]?.[selectedPrimitive];return relationshipBadge(useCaseRelationship,useCaseTitles[selectedUseCase])+relationshipBadge(primitiveRelationship,primitiveTitles[selectedPrimitive])}}
 function show(note){{const tags=(note.tags||[]).map(t=>`<span class="tag">#${{escapeHtml(t)}}</span>`).join('');const ratings=Object.entries(note.ratings||{{}}).map(([name,r])=>`<div class="rating"><strong>${{escapeHtml(name)}}: ${{escapeHtml(r.value)}}/100</strong><br>${{escapeHtml(r.note)}}</div>`).join('');content.innerHTML=`<p class="intro">${{escapeHtml(note.kind)}}${{relationshipBadges(note)}}</p><h2 id="dialog-title">${{escapeHtml(note.title)}}</h2><p>${{escapeHtml(note.summary)}}</p><p>${{tags}}</p>${{ratings}}<p><a href="${{escapeHtml(note.url)}}">Read the full Markdown note on GitHub</a></p>`;dialog.showModal()}}
 function showCluster(items){{const hasSelection=Boolean(selectedUseCase||selectedPrimitive);const isRelated=note=>noteMatchesSelection(note.id);const ordered=hasSelection?[...items.filter(isRelated),...items.filter(note=>!isRelated(note))]:[...items];content.innerHTML=`<p class="intro">${{ordered.length}} notes at this position</p><h2 id="dialog-title">Select a note</h2>${{ordered.map((note,i)=>`<button class="picker-item" data-index="${{i}}"><span class="picker-kind ${{escapeHtml(note.kind)}}">${{escapeHtml(note.kind)}}</span><strong>${{escapeHtml(note.title)}}</strong>${{relationshipBadges(note)}}<br><small>${{escapeHtml(note.summary)}}</small></button>`).join('')}}`;dialog.showModal();content.querySelectorAll('.picker-item').forEach(b=>b.onclick=()=>show(ordered[Number(b.dataset.index)]))}}
...
def main() -> int:
    ...
    try:
        output = generate_html(load_notes(), load_plots(), load_primitives(), load_focus_use_cases())
