...
<script>const plots={data};const primitiveMemberships={membership_data};const useCaseMemberships={use_case_membership_data};const dialog=document.querySelector('#detail'),content=document.querySelector('#content');let selectedUseCase=null,selectedPrimitive=null;document.querySelector('.close').onclick=()=>dialog.close();
 ...
 function updateMarkers(){{const hasSelection=Boolean(selectedUseCase||selectedPrimitive);document.querySelectorAll('.marker').forEach(m=>{{if(m.dataset.noteIds){{const noteIds=JSON.parse(m.dataset.noteIds);const relatedCount=hasSelection?noteIds.filter(noteMatchesSelection).length:0;m.textContent=relatedCount?`${{relatedCount}}/${{noteIds.length}}`:String(noteIds.length);m.setAttribute('aria-label',relatedCount?`${{relatedCount}} of ${{noteIds.length}} related notes at this position`:`${{noteIds.length}} notes at this position`);m.classList.toggle('related',relatedCount>0);m.classList.toggle('dimmed',Boolean(hasSelection&&!relatedCount))}}else{{const related=noteMatchesSelection(m.dataset.id);m.classList.toggle('related',hasSelection&&related);m.classList.toggle('dimmed',hasSelection&&!related)}}}})}}
