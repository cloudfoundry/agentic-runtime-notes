...
function refreshMarkers(){{document.querySelectorAll('.marker[data-id]').forEach(m=>{{const related=Boolean(selectedPrimitive&&primitiveMemberships[m.dataset.id]?.[selectedPrimitive]);m.classList.toggle('related',related);m.classList.toggle('dimmed',Boolean(selectedPrimitive&&!related))}})}}
